...
**58 decoders covering 178 model names across 4 vendors.**
...
| LDS02 | `'lds02'` | `dragino.lds02` | — | 10, 7 | Door sensor with open count and last open duration |
| LHT52 | `'lht52'` | `dragino.lht52` | — | 2, 3 | Temperature and humidity sensor with optional DS18B20 probe |
| LHT65 | `'lht65'` | `dragino.lht65` | `LHT65N` | 2, 3 | Temperature and humidity sensor with external probe input (DS18B20, ADC, counter, interrupt) |
...
| LWL02 | `'lwl02'` | `dragino.lwl02` | — | 10, 7 | Water leak sensor with leak count and last leak duration |
...
| CT103 | `'ct103'` | `milesight.ct103` | `CT101`, `CT105` | — | Smart current transformer (CT101/CT103/CT105 share the format) |
...
| VS351 | `'vs351'` | `milesight.vs351` | — | — | Mini AI thermopile people counter |
...
| CT103 | `total_current` (Ah), `current` (mA), `current_status`, `temperature` (°C), `temperature_status`, `energy` (kWh), `current_max` (mA), `current_min` (mA), `current_alarm`, `current_over_range_alarm`, `temperature_alarm` |
...
| VS351 | `battery` (%), `temperature` (°C), `total_counter_in` (count), `total_counter_out` (count), `periodic_counter_in` (count), `periodic_counter_out` (count), `temperature_alarm`, `total_counter_alarm`, `periodic_counter_alarm` |
...
| RA02A | `'ra02a'` | `netvox.ra02a` | — | 6, 7 | Smoke detector with temperature and fixed 60 °C high-temperature alarm |
...
| RA02A | `battery_voltage` (V), `battery_low`, `fire_alarm`, `temperature_alarm`, `temperature` (°C) |
