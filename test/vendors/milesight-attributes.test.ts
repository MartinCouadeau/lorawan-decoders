...
describe('Milesight attribute channels', () => {
  it('formats the version and identity channels, and keeps them out of telemetry', () => {
    const uplink = run(
      'Milesight', 'EM400-TLD',
  ...
  it('reads the 8-byte serial number, not the 2 bytes the vendor README claims', () => {
    ...
    expect(uplink.telemetry).toEqual({ battery: 110 });
  });

  it('decodes the GS301 calibration result struct', () => {
    const uplink = run('Milesight', 'GS301', '07ea' + '01' + '01' + '6400' + '00');
    expect(uplink.attributes).toEqual({
      calibration_sensor: 'h2s', calibration_type: 'manual', calibration_value: 0.1,
    });
    expect(uplink.telemetry).toEqual({ calibration_result: 'success' });
  });
});
